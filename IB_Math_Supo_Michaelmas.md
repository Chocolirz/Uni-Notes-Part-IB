...
\end{split}
$$
Or, more clearly written it piecewise!!!
$$
\begin{split}
...
### Sheet 1 Q6
Never write $x_i^2$ in suffix notations, write $x_i x_i$ instead - Do NOT mislead yourself! 
...
The first op-amp is a oscillator whose inputs is sign wave and output is square wave - this is a digitiser used in computers, for example, $-15\ {\rm V}$ for $0$ and $15\ {\rm V}$ for $1$. 
...
$$
\int_V\nabla p\ \mathrm dV=\oint_{\partial V}p\ \mathrm dS
$$
because $\vec a$ is an arbitrary vector, not anything else.
...
### Sheet 3 Q7
Rotate matrices in the same way the **preserves the rotation angle** of vectors in order to preserve the scalar produced by dotting vectors. Therefore if $x^\prime = Rx$, $M^{\prime}$ for $x^\prime$ is $RMR^{-1}$. 
